...
export class AttributeDef {
	...
	computeCost(character: CharacterGURPS, value: number, cost_reduction: number, size_modifier: number): number {
		...
		if (size_modifier > 0 && this.cost_adj_percent_per_sm > 0 && !(this.def_id == "hp" && character.settings.damage_progression == "knowing_your_own_strength")) cost_reduction = size_modifier * this.cost_adj_percent_per_sm;
		if (cost_reduction > 0) {

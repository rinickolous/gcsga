...
export class SpellPrereq extends BasePrereq {
	...
	satisfied(
		...
		let tech_level = "";
		if (
			exclude instanceof SpellGURPS ||
			exclude instanceof RitualMagicSpellGURPS
		)
		...
		if (!satisfied) {
			...
			if (this.sub_type == "college_count") {
				...
				tooltip.push(" ");
				tooltip.push(
					`gurps.prereqs.criteria.${this.quantity?.compare}`,
				);
				if (this.quantity?.qualifier == 1)
					tooltip.push(`gurps.prereqs.spell.one`);
				else tooltip.push(`gurps.prereqs.spell.many`);
				tooltip.push(" ");
				if (this.sub_type == "any")
					tooltip.push(`gurps.prereqs.spell.any`);
				else {
					if (this.sub_type == "name")
						tooltip.push(`gurps.prereqs.spell.name`);
					else if (this.sub_type == "tag")
						tooltip.push(`gurps.prereqs.spell.tag`);
					else if (this.sub_type == "college")
						tooltip.push(`gurps.prereqs.spell.college`);
					tooltip.push(
						`gurps.prereqs.criteria.${this.qualifier?.compare}`,
					);

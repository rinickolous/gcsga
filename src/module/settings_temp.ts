import { CharacterProfile, DamageProgression } from "@actor/character/data";
import { HitLocationTable } from "@actor/character/data";
import { Attribute, AttributeDef } from "@module/attribute";
import { DisplayMode, LengthUnits, RollGURPS, RollRange, WeightUnits } from "@module/data";
...
interface provider {
	sheet: {
		...
		hit_locations: Omit<HitLocationTable, "recursiveLocations">;
	};
	general: {
		...
		hit_locations: {
			...
			locations: [
				...
				},
				{
					...
					description: "Reduce the wounding multiplier of large piercing (pi+), huge piercing (pi++), and impaling (imp) damage to x1. Any major wound (loss of over ½ HP from one blow) cripples the limb. Damage beyond that threshold is lost.",
					calc: {
						roll_range: new RollRange("13-14"),
						dr: {
							all: 0
						}
					}
				...
				},
				{
					...
					calc: {
						roll_range: new RollRange("17-18"),
						dr: {
							all: 0
						}
					}

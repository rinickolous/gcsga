...
export class SkillDefault {
	...
	skillLevel(
		...
		switch (this.type) {
			case "parry":
				...
				if (best != Math.max())
					best = best / 2 + 3 + actor.calc.parry_bonus;
				return this.finalLevel(best);
			case "block":
				best = this.best(actor, require_points, excludes);
				if (best != Math.max())
					best = best / 2 + 3 + actor.calc.block_bonus;
				return this.finalLevel(best);
			case "skill":
				return this.finalLevel(
					this.best(actor, require_points, excludes),
				);
			default:
				return this.skillLevelFast(
					actor,
					require_points,
					excludes,
					rule_of_20,
	...
	best(
		...
		let best = Math.max();
		for (const s of actor.skillNamed(
			this.name!,
			this.specialization || "",
			require_points,
			excludes,
	...
	skillLevelFast(
		...
		switch (this.type) {
			...
			case gid.Parry:
				...
				if (best != Math.max())
					best = Math.floor(best / 2) + 3 + actor.calc.parry_bonus;
				return this.finalLevel(best);
			case gid.Block:
				best = this.bestFast(actor, require_points, excludes);
				if (best != Math.max())
					best = Math.floor(best / 2) + 3 + actor.calc.block_bonus;
				return this.finalLevel(best);
			case gid.Skill:
				return this.finalLevel(
					this.bestFast(actor, require_points, excludes),
				);
			default:
	...
	bestFast(
		...
		let best = Math.max();
		for (let sk of actor.skillNamed(
			this.name!,
			this.specialization || "",
			require_points,
			excludes,
